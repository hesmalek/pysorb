...
"""
pysorb_mix: A Python module for adsorption mixture modelling.
...
def iast_core(p_part, model_ids_in, p1_in, p2_in, p3_in, max_iter=IAST_MAX_ITER, tol=IAST_TOL):
    ...
    x, err, ok = solve_iast(
        p_part, qs_all, bs_all, fs_all, K_all, model_ids, n_comp, n_site,
        max_iter=max_iter, tol=tol
    )
    qT = total_loading(x, p_part, qs_all, bs_all, fs_all, K_all, model_ids, n_comp, n_site)
    q  = component_loadings_iast(x, qT, n_comp)
